        ...
        >
            ...
            <div className="p-3 flex flex-col flex-1">
                <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[9px] text-[#001A2C] font-black uppercase tracking-widest truncate">{product.category}</span>
                </div>
                <h3 className="text-[11px] font-bold text-[#001A2C] line-clamp-2 min-h-[30px] mb-2 leading-tight uppercase tracking-tight">
                    {product.name}
                </h3>
                <div className="mt-auto">
                    <div className="text-[18px] font-black text-[#008B8B] leading-none">
                        ${product.casePrice.toFixed(2)}
                    </div>
                    <div className="text-[10px] text-[#001A2C] font-bold mb-1 mt-1 uppercase">({product.unitPrice.toFixed(2)} / unit)</div>
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between border border-slate-300 rounded-sm overflow-hidden bg-slate-50 focus-within:border-teal-accent transition-all">
                            <button
                                ...
                                className="px-3 py-1.5 hover:bg-slate-200 text-[#001A2C] font-black transition-colors border-r border-slate-200 cursor-pointer active:bg-slate-300"
                            >-</button>
                            <input
                                className="w-12 text-center border-none text-[11px] font-black p-0 outline-none bg-transparent text-[#001A2C] focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                type="number"
                                value={quantity}
                            ...
                            <button
                                ...
                                className="px-3 py-1.5 hover:bg-slate-200 text-[#001A2C] font-black transition-colors border-l border-slate-200 cursor-pointer active:bg-slate-300"
                            >+</button>
                        </div>
                        <motion.button
                            whileTap={{ scale: 0.95 }}
